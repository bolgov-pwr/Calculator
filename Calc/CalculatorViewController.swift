...
class CalculatorViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let id = segue.identifier,
            id == StoryBoard.ShowGraph,
            let vc = destination as? GraphViewController {
            vc.yForX = { [weak weakSelf = self] x in
                weakSelf?.variableValues["M"] = x
                return (weakSelf?.brain.evauate(using: weakSelf?.variableValues).result)!
            }
            vc.navigationItem.title = "y = " + brain.evauate(using: variableValues).description
        }
    }
    override func shouldPerformSegue(withIdentifier identifier: String, sender: Any?) -> Bool {
        if identifier == StoryBoard.ShowGraph {
            let result = brain.evauate()
